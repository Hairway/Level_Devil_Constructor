...
export default class Game extends IMPION.ComponentEmpty {
    ...
    initGame = () => {	
		...
		//this.#app.view3d.renderer.shadowMap.autoUpdate = false;
		//this.#app.view3d.renderer.shadowMap.needsUpdate = true;
	...
	updateLabelHero(){		
		...
		this.#point3d.position.copy( this.components["Crowd"].characters[0].position );
    ...
    enterframe = ( timeDelta )=>{		
		...
		let hero = this.components["Crowd"].characters[0];
		
		if(hero.actionCurrent == "run"){
		...
		for(let i = 1; i<this.components["Crowd"].characters.length; i++){
			...
			character.tmAI++;
			if(character.tmAI > 200){
				character.tmAI = Math.randomInteger(0, 100);
				
				character.commandToPoints(this.components["PathFinder"], [
					new IMPION.Vector3(
						20-20*2*Math.random(),
						0,
						20-20*2*Math.random(),
