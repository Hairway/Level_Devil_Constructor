...
export default class CreateObjects3D{
	...
	constructor({app, gameComponent}){
		...
		gsap.to(this.components["World"]["raft"].position,	0.8,	{delay:0.0, overwrite: "none", y:"-=0.2", repeat:-1, yoyo:true, ease:"sine.inOut"});
		gsap.to(this.components["World"]["raft"].rotation,	1.2,	{delay:0.0, overwrite: "none", x:"-=0.1", repeat:-1, yoyo:true, ease:"sine.inOut"});
		gsap.to(this.components["World"]["raft"].rotation,	1.4,	{delay:0.0, overwrite: "none", z:"-=0.1", repeat:-1, yoyo:true, ease:"sine.inOut"});
		...
		this.components["EmitterDust"] = new IMPION.Emitter3d({
			particles			: [ new THREE.Sprite( this.materials["vfxDust"] ) ],
			quantity			: 100,
	...
	//	this.components["Hero"]["Label"] = new THREE.Sprite( this.materials["labelHero"] );
	...
	//	this.#app.physics3d.addCharacter( this.components["Hero"] );
		...
		this.components["Crowd"] = new IMPION.Crowd3d({
			app			: this.#app,
			name		: "people",			
			quantity	: 1000,
			dispose		: ["weapon"],
		...
		this.#app.view3d.scene.add( this.components["Crowd"] );

		for(let i = 0; i<this.components["Crowd"].characters.length; i++){
			this.components["Crowd"].characters[0].action["run"].speed = 2;
		}
		...
		for(let i = 1; i<this.components["Crowd"].characters.length; i++){
			let character = this.components["Crowd"].characters[i];
			
			character.forceRun *= 0.5; 
			...
			character.positionTo.copy(pos);
			
			//this.#app.physics3d.addCharacter( character );
		}	
		
		this.components["Crowd"].characters[0].position.copy( this.components["World"]["point_hero"].position ); 
		this.components["Crowd"].characters[0].positionTo.copy( this.components["Crowd"].characters[0].position ); 
		this.components["Crowd"].characters[0].isAI = false;
		this.#app.physics3d.addCharacter( this.components["Crowd"].characters[0] );
		...
		this.components["Camera"] = new IMPION.CameraTargets({
			...
			position 		: new THREE.Vector3( -12, 22, 12),
			rate			: 0.85,
			...
			targets 		: {
				"hero" : {object: this.components["Crowd"].characters[0], weight:1}
			}
        });		
